...
- ▶️ **Этап 3** — прогресс, подбор набора, упражнения без LLM; код готов и проверен
  локально (wrangler + Neon), осталась ручная проверка на проде.
...
**Готово когда:** ⏳ проверить на `vocab.kunitcan.online` — собрать набор (авто и по теме),
пройти сессию тапами (все 3 типа + «подсмотреть»), увидеть итог; повторный заход в
авто-режим показывает изменившийся приоритет. Локально (wrangler + Neon) всё проверено.
