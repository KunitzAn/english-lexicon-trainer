...
export function buildPrompt(senses: SenseForGen[]): {
  ...
  const system = [
    ...
    'this is THE sense to train), plus optional "part_of_speech", "definition_en", "example".',
    'CRITICAL: the sentence must use "word" in EXACTLY the sense given by "translation"',
    '(and matching "definition_en"/"example" when present). If the word is polysemous,',
    'do NOT drift to another sense. Put the word in a natural, correct collocation that a',
    'careful native speaker would actually use for this sense — no awkward, forced, or',
    'borderline pairings. The surrounding words must fit that specific meaning.',
    '',
    ...
    ' "bank":[4 single English words incl. answer; distractors same part of speech, each a real word,',
    ' wrong in THIS context and this sense]}.',
    '',
    'kind "clickable": ONE short sentence (about 12–20 words) that uses the target English word (inflected forms allowed).',
    ' The context must make the intended sense unambiguous. Fields: {"kind":"clickable","sense_id":<int>,"text":<string>,',
